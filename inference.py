...
def parse_args():
    ...
    parser.add_argument('--image-dir', type=str, default='imagenes_validacion',
                        help='Directory with images to caption')
    parser.add_argument('--model-path', type=str, required=True,
                        help='Path to trained model checkpoint')
    parser.add_argument('--vocab-file', type=str, default='vocab.pkl',
    ...
    parser.add_argument('--beam-search', action='store_true',
                        help='Use beam search instead of greedy')
...
def generate_captions(model, image_dir, vocab, transform, device, beam_search=False, beam_width=3, max_length=20):
...
def visualize_results(results, output_dir, model_name="Model"):
    ...
    plt.suptitle(f'Image Captioning Results - {model_name}', 
                 fontsize=16, y=0.995)
    plt.tight_layout()
    
    output_path = os.path.join(output_dir, f'results_{model_name.lower().replace(" ", "_")}.png')
...
def save_results_txt(results, output_dir, model_name="Model"):
    ...
    output_path = os.path.join(output_dir, f'captions_{model_name.lower().replace(" ", "_")}.txt')
...
def main():
    ...
    results = generate_captions(
        model, args.image_dir, vocab, transform, device,
